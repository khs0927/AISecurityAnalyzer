...
import aiAnalysisRouter from "./routes/ai_analysis";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // HAIM AI 분석 라우터 등록
  app.use('/api/haim', aiAnalysisRouter);
  
  // OpenAI 기반 AI 라우터 등록
  app.use('/api/openai', aiRouter);
  app.use('/api/ai', aiRouter);
  app.use('/api/v1', aiRouter); // v1 API 경로 추가
  ...
  // AI-powered consultation endpoints using HAIM model
  app.post('/api/ai-chat', async (req, res) => {
    try {
      ...
      // 리디렉션: HAIM 모델 기반 AI 상담 API로 리다이렉트
      // 이 방식으로 기존 코드를 유지하면서 HAIM 모델을 사용할 수 있습니다
      const response = await fetch(`http://localhost:${process.env.PORT || 5000}/api/haim/consultation`, {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        throw new Error(`HAIM API error: ${errorData.error || response.status}`);
      }
      
      const data = await response.json();
      
      res.json({ aiResponse: data.aiResponse });
    } catch (error) {
