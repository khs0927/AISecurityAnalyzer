              ...
              index === 0 ? (
                ...
                >
                  <div 
                    ...
                    style={buttonStyle}
                  >
                    <Phone className="text-white w-4 h-4 mr-1" />
...
export const HeartDiagnosisSubNav = () => {
  ...
  // AI 헬퍼 경로 확인
  const isAiHelper = location === '/ai-voice-consultation' || location === '/ai-consultation';
  
  // AI 헬퍼 페이지일 경우 빈 div만 반환 (탭 삭제)
  if (isAiHelper) {
    return <div className="mb-4"></div>;
  }
  
  // 다른 페이지의 경우 기존 탭 메뉴 표시
  const subNavItems = [
    ...
    { path: '/ai-voice-consultation', icon: MessageSquare, label: 'AI 헬퍼' }
  ];

  return (
    <div className="mb-4 pt-2">
      <div className="grid grid-cols-3 gap-2 bg-white shadow-sm rounded-xl border border-[#FFD6D6] p-2">
        {subNavItems.map((item) => {
          ...
          return (
            <Link href={item.path} key={item.path}>
              <div className={`flex items-center justify-center py-1.5 px-2 rounded-lg transition-all duration-300 ${
                isActive ? 'bg-[#FF0000] text-white' : 'text-black hover:bg-[#FFF5F5]'
              }`}>
                <IconComponent className={`h-4 w-4 mr-1 ${isActive ? 'text-white' : 'text-[#FF0000]'}`} />
